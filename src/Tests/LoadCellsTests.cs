...
[TestFixture]
public class LoadCellsTests
{
    [Test]
    public void LoadXlsx_ValidXlsx_ReturnsWorkbook()
    {
        // Arrange
        using var stream = CreateValidXlsxStream();

        // Act
        var workbook = CellsExtensions.Load(stream);
    ...
    [Test]
    public void LoadXlsx_ValidXlsxWithData_PreservesContent()
    {
        // Arrange
    ...
    [Test]
    public void LoadXlsx_ValidXlsxWithMultipleSheets_PreservesAllSheets()
    {
        // Arrange
    ...
    [Test]
    public void LoadXlsx_XlsFormat_ThrowsException()
    {
        // Arrange
        using var stream = CreateXlsStream();

        // Act & Assert
        var exception = Assert.Throws<Exception>(() => CellsExtensions.Load(stream))!;
    ...
    [Test]
    public void LoadXlsx_CsvFormat_ThrowsException()
    {
        // Arrange
        using var stream = CreateCsvStream();

        // Act & Assert
        var exception = Assert.Throws<CellsException>(() => CellsExtensions.Load(stream))!;
        Assert.That(exception.Message, Does.Contain("Unsupported file format"));
    }

    [Test]
    public void LoadXlsx_CorruptFile_ThrowsException()
    {
        // Arrange
    ...
    [Test]
    public void LoadXlsx_EmptyStream_ThrowsException()
    {
        // Arrange
        using var stream = new MemoryStream();

        // Act & Assert
        Assert.Throws<Exception>(() => CellsExtensions.Load(stream));
    }

    [Test]
    public void LoadXlsx_StreamNotAtBeginning_LoadsCorrectly()
    {
        // Arrange
